...
//	if (!defined('ADODB_DIR')) define('ADODB_DIR',dirname(__FILE__));
	if (!defined('ADODB_DIR')) define('ADODB_DIR',"$ruta_raiz/adodb/");
	//==============================================================================================	
	// GLOBAL VARIABLES
	//==============================================================================================	
	...
	*/
	...
	GLOBAL 
		 ...
		 */
		$ADODB_vers = 'V4.53 14 Sept 2004  (c) 2000-2004 John Lim (jlim#natsoft.com.my). All rights reserved. Released BSD & LGPL.';
	...
	var $numToString_func = ""; ///Funci�n por defecto para convertir tipo de dato num�rico a texto -- cambia a STR para MsSql
	var $ltrim_func = ""; /// Funci�n para remover los espacios de la parte izaquierda de un String -- cambia a ltrim para MsSql
	...
	*/
	...
	function Replace($table, $fieldArray, $keyCol, $autoQuote=false, $has_autoinc=false, $saveLog=false, $showQuery=false)
	{
		$insert = "";
		$update = "";
		global $ADODB_INCLUDED_LIB;
		...
		$resp = _adodb_replace($this, $table, $fieldArray, $keyCol, $autoQuote, $has_autoinc, $insert, $update);
        if (trim($insert)=="") $cadena = $update; else $cadena = $insert;
        $this->querySql = $cadena . " - " . $this->ErrorMsg();
//		file_put_contents("/tmp/log1.txt", "$update - $insert -");
        if (isset($_SESSION)) {
            $grabar_logs = $_SESSION['grabar_logs'];
            $usua_codi = $_SESSION['usua_codi'];
            $mostrar_logs = $_SESSION['mostrar_logs'];
        } else {
            $grabar_logs = 2;
            $usua_codi = "-1";
            $mostrar_logs = 0;
        }
		if ($saveLog and ($grabar_logs==2 or ($grabar_logs==1 and $resp==0))) {
			if (trim($insert)=="") $cadena = addslashes($update); else $cadena = addslashes($insert);
			$fecha = $this->sysTimeStamp;
			$sql = "insert into log (fecha, usua_codi, tabla, sentencia, tipo) 
				values ($fecha, $usua_codi,'$table', E'$cadena', $resp)";
			$this->Execute($sql);
		}
		if ($mostrar_logs==2 or (($mostrar_logs==1 and $resp==0))) {
			echo $this->querySql;
		}
                if($showQuery==true)
                    echo $insert.$update;
	...
	*/
	...
	/**
	 * A la hora de concatenar campos en un query, algunas bases de datos como Oracle combierten automaticamente el tipo de dato a String
	 * otras como MsSql necesitan una conversi�n expl�sita del campo  a texto. Esta funci�n obtiene en nombre de la rutina
	 * de conversi�n para tipos num�ricos 
	 *
	 * @param n	el campo num�rico a convertir
	 * 
	 * Uso: $db->numToString($str1);
	 * 
	 * @return el campo con la funci�n de conversi�n
	 */ 	 
	function numToString($n)
	{	
		if (empty($n) && $n !== 0) return 'null';
		
		return $this->numToString_func."(".$n.")";
	}
	
		/**
	 * Funci�n para remover los espacios de la parte izquierda de un tring
	 *
	 * @param s	el string a procesar
	 * 
	 * Uso: $db->ltrim($str1);
	 * 
	 * @return el string con la funci�n de remover los espacioa a izquierda
	 */ 	 
	function ltrim($s)
	{	
		if (empty($s) && $s !== 0) return 'null';
		return $this->ltrim_func."(".$s.")";
	}
	...
	*/
	...
	{	
		if (!$magic_quotes) {
			...
			return  "E'".str_replace("'",$this->replaceQuote,$s)."'";
		}
