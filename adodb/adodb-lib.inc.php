...
function _adodb_replace(&$zthis, $table, $fieldArray, $keyCol, $autoQuote, $has_autoinc, &$insert, &$update)
{
...
/*	 if (preg_match("/^\s*SELECT\s+DISTINCT/is", $sql) || preg_match('/\s+GROUP\s+BY\s+/is',$sql)) {
		...
		if ($zthis->dataProvider == 'oci8') {
			...
			if (substr($info['version'],0,3) >= 7.1) { // good till version 999
                //Línea 193 comentada por M. Haro
				//$rewritesql = preg_replace('/(\sORDER\s+BY\s.*)/is','',$sql);
...
//      Línea 206 comentada por M. Haro
//		$rewritesql = preg_replace('/(\sORDER\s+BY\s.*)/is','',$rewritesql);
	}/**/
    // Código incluido por M. Haro hasta línea 220 revisión 456
    // El código fue incluido para mejorar el rendimiento eliminando la ejecución de la función ver_usuarios() en el count del paginador
    $rewritesql = "SELECT COUNT(*) FROM ($sql) _ADODB_ALIAS_";
	$tmp = strtolower($rewritesql);
    while (strpos($tmp,'ver_usuarios') !== false) {
        $pos1 = strpos($tmp,'ver_usuarios');
        $pos2 = strpos($tmp,'(',$pos1);
        $pos3 = strpos($tmp,',',$pos2);
        $pos4 = strpos($tmp,')',$pos3);
        $rewritesql = substr($rewritesql, 0, $pos1) . substr($rewritesql,$pos2, $pos3-$pos2) . substr($rewritesql, $pos4);
        $tmp = strtolower($rewritesql);
    }
    $rewritesql = str_replace('*LIMIT**OFFSET*','',$rewritesql);
//    echo "$rewritesql<hr>";
