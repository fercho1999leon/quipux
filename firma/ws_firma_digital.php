 ...
 *------------------------------------------------------------------------------
 **/
//IMPORTAMOS LA LIBRERIA PARA PDF 
require_once("tcpdf/tcpdf.php");
require_once("fpdi/fpdi.php");

function grabar_archivos_firmados($usuario, $radi_nume, $archivo, $firma, $passwordFirma)
{
    //$pdf = new TCPDF();

    /*// Configuración de la firma invisible
    $passphrase = $passwordFirma;
    $reason = 'FIRMA DOCUMENTO QUIPUX ISTRED';
    $location = 'DAULE';

    $pdf->AddPage();

    // Configuración de la firma invisible
    $pdf->setSourceFile(StreamReader::createByString($archivo));
    $tplIdx = $pdf->importPage(1);
    $pdf->useTemplate($tplIdx);
    // Decodificar el certificado P12 base64
    $certificado = base64_decode($firma);
    $pkcs12 = $certificado;
    // Vertificar certificado P12
    if (openssl_pkcs12_read($pkcs12, $certs, $password)) {
        $cert = array_shift($certs);
        $x509 = openssl_x509_read($cert);
        $private_key = openssl_pkey_get_private($pkcs12, $password);
    } else {
        throw new Exception('La contraseña del certificado es incorrecta');
    }

    // Cargar el certificado P12 en el objeto TCPDF
    $pdf->setSignature($certificado, $certificado, $passphrase, '', $reason, $location);
    $archivo = base64_encode($pdf->Output("documento.pdf", 'S'));*/
    /*
...
$sServer = new SoapServer("https://pruebas.quipux.itred.edu.ec/firma/wsFirma.wsdl");
