...
class ADODB_postgres7 extends ADODB_postgres64 {
    ...
    // Función alterada por M. Haro en la revisión de quipux 456
    // El código fue incluido para mejorar el rendimiento eliminando la ejecución de la función ver_usuarios()
    // para todos los registros obtenidos en el query, ejecutandola solo para los registros obtenidos despues del limit.
    // se colocan los parámetros de límit y offset reemplazando la cadena *LIMIT**OFFSET*

	 function &SelectLimit($sql,$nrows=-1,$offset=-1,$inputarr=false,$secs2cache=0) 
	 {
		 $offsetStr = ($offset >= 0) ? " OFFSET $offset" : '';
		 $limitStr  = ($nrows >= 0)  ? " LIMIT $nrows" : '';
         if (strpos($sql,'*LIMIT**OFFSET*')===false) {
             $isql = $sql."$limitStr$offsetStr";
         } else {
             $isql = str_replace('*LIMIT*', $limitStr, str_replace('*OFFSET*', $offsetStr, $sql));
         }
//         echo $isql."<hr>";
		 if ($secs2cache)
		  	$rs =& $this->CacheExecute($secs2cache,$isql,$inputarr);
		 else
		  	$rs =& $this->Execute($isql,$inputarr);
