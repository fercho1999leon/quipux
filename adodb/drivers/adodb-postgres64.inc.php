...
WHERE relkind = 'r' AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
	...
	var $num_rows_result = 0; //Numero de registros encontrados en el select
...
//              echo str_replace("<", "&lt;", $sql)."<br>";
		...
		*/
			...
			$rez = pg_exec($this->_connectionID,$sql);
                        if ($rez && $this->_log_full_backup && trim($sql)!="" &&
                                (strtoupper(substr(trim($sql), 0, 2))!="SE" || strtoupper(substr(trim($sql), 0, 14))=="SELECT NEXTVAL")) {
                            $sql_log_full = "insert into log_full_backup (sentencia) values (E'".base64_encode($sql)."')";
                            $rez_log_full = pg_exec($this->_connectionID,$sql_log_full);
                        }
		}
		...
		if ($rez && pg_numfields($rez) <= 0) {
			...
			return true;
                ...
                $this->num_rows_result = pg_num_rows($rez);
		return $rez;
	}
